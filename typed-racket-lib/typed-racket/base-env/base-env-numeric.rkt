...
(begin
  (require
   ...
   (only-in (types abbrev numeric-tower)
  ...
  (define (int-type? t)
    (for/or ([t* (in-list all-int-types)])
      (type-equal? t t*)))
  ...
  (define (commutative-equality/strict-filter general specific)
    ...
    (define (commutative-equality-int/strict-filter general specific)
      (list (-> general specific B : (-FSb= (-filter specific 0) (-not-filter specific 0)))
            (-> specific general B : (-FSb= (-filter specific 1) (-not-filter specific 1)))))
  ...
  (define fx+-type
    (lambda ()
      (fx-from-cases
       ...
       (isum-> -PosByte -Byte -PosIndex)
       (isum-> -Byte -PosByte -PosIndex)
       (isum-> -Byte -Index -Index) ;; BOOKMARK
       ;; in other cases, either we stay within fixnum range, or we error
       (isum-> -Pos -Nat -PosFixnum)
       (isum-> -Nat -Pos -PosFixnum)
       (isum-> -Nat -Nat -NonNegFixnum)
       (isum-> -NegInt -One -NonPosFixnum)
       (isum-> -One -NegInt -NonPosFixnum)
       (isum-> -NegInt -NonPosInt -NegFixnum)
       (isum-> -NonPosInt -NegInt -NegFixnum)
       (isum-> -NonPosInt -NonPosInt -NonPosFixnum)
       (isum-> -Int -Int -Fixnum))))
  (define fx--type
    (lambda ()
      (fx-from-cases
       ...
       (idiff-> -PosInt -One -NonNegFixnum)
       (idiff-> -NegInt -Nat -NegFixnum)
       (idiff-> -NonPosInt -PosInt -NegFixnum)
       (idiff-> -NonPosInt -Nat -NonPosFixnum)
       (idiff-> -PosInt -NonPosInt -PosFixnum)
       (idiff-> -Nat -NegInt -PosFixnum)
       (idiff-> -Nat -NonPosInt -NonNegFixnum)
       (idiff-> -Int -Int -Fixnum))))
  (define fx*-type
    (lambda ()
      (fx-from-cases
  ...
  (define fx=-type
    (lambda ()
      (fx-from-cases
       ...
       (commutative-equality-int/strict-filter -Int -Zero)
       (map (lambda (t) (commutative-equality-int/filter -Int t))
            (list -One -PosByte -Byte -PosIndex -Index -PosFixnum -NonNegFixnum -NegFixnum -NonPosFixnum))
       (-> -Int -Int B : (-FSb= -top -top)))))
  (define fx<-type
    (lambda ()
      (fx-from-cases
       (-> -Int -One B : (-FSb< (-filter -NonPosFixnum 0) (-filter -PosFixnum 0)))
       (-> -Int -Zero B : (-FSb< (-filter -NegFixnum 0) (-filter -NonNegFixnum 0)))
       (-> -Zero -Int B : (-FSb< (-filter -PosFixnum 1) (-filter -NonPosFixnum 1)))

       (-> -Byte -PosByte B : (-FSb< -top (-filter -PosByte 0)))
       (-> -Byte -Byte B : (-FSb< (-filter -PosByte 1) -top))
       (-> -Pos -Byte B : (-FSb< (-and (-filter -PosByte 0) (-filter -PosByte 1)) -top))
       (-> -Byte -Pos B : (-FSb< -top (-and (-filter -PosByte 0) (-filter -PosByte 1))))
       (-> -Byte -Nat B : (-FSb< -top (-filter -Byte 1)))
       (-> -Index -PosIndex B : (-FSb< -top (-filter -PosIndex 0)))
       (-> -Index -Index B : (-FSb< (-filter -PosIndex 1) -top))
       (-> -Pos -Index B : (-FSb< (-and (-filter -PosIndex 0) (-filter -PosIndex 1)) -top))
       (-> -Index -Pos B : (-FSb< -top (-and (-filter -PosIndex 0) (-filter -PosIndex 1))))
       (-> -Nat -Byte B : (-FSb< (-and (-filter -Byte 0) (-filter -PosByte 1)) -top))
       (-> -Nat -Index B : (-FSb< (-and (-filter -Index 0) (-filter -PosIndex 1)) -top))
       (-> -Index -Nat B : (-FSb< -top (-filter -Index 1)))
       ;; general integer cases
       (-> -Int -PosInt B : (-FSb< -top (-filter -PosFixnum 0)))
       (-> -Int -Nat B : (-FSb< -top (-filter -NonNegFixnum 0)))
       (-> -Nat -Int B : (-FSb< (-filter -PosFixnum 1) -top))
       (-> -Int -NonPosInt B : (-FSb< (-filter -NegFixnum 0) -top))
       (-> -NegInt -Int B : (-FSb< -top (-filter -NegFixnum 1)))
       (-> -NonPosInt -Int B : (-FSb< -top (-filter -NonPosFixnum 1)))
       (-> -Int -Int B : (-FSb< -top -top)))))
  (define fx>-type
    (lambda ()
      (fx-from-cases
       (-> -One -Int B : (-FSb> (-filter -NonPosFixnum 1) (-filter -PosFixnum 1)))
       (-> -Zero -Int B : (-FSb> (-filter -NegFixnum 1) (-filter -NonNegFixnum 1)))
       (-> -Int -Zero  B : (-FSb> (-filter -PosFixnum 0) (-filter -NonPosFixnum 0)))

       (-> -PosByte -Byte B : (-FSb> -top (-filter -PosByte 1)))
       (-> -Byte -Byte B : (-FSb> (-filter -PosByte 0) -top))
       (-> -Byte -Pos B : (-FSb> (-and (-filter -PosByte 0) (-filter -PosByte 1)) -top))
       (-> -Pos -Byte B : (-FSb> -top (-and (-filter -PosByte 0) (-filter -PosByte 1))))
       (-> -Byte -Nat B : (-FSb> (-and (-filter -PosByte 0) (-filter -Byte 1)) -top))
       (-> -PosIndex -Index B : (-FSb> -top (-filter -PosIndex 1)))
       (-> -Index -Index B : (-FSb> (-filter -PosIndex 0) -top))
       (-> -Index -Pos B : (-FSb> (-and (-filter -PosIndex 0) (-filter -PosIndex 1)) -top))
       (-> -Pos -Index B : (-FSb> -top (-and (-filter -PosIndex 0) (-filter -PosIndex 1))))
       (-> -Index -Nat B : (-FSb> (-and (-filter -PosIndex 0) (-filter -Index 1)) -top))
       (-> -Nat -Byte B : (-FSb> -top (-filter -Byte 0)))
       (-> -Nat -Index B : (-FSb> -top (-filter -Index 0)))
       ;; general integer cases
       (-> -PosInt -Int B : (-FSb> -top (-filter -PosFixnum 1)))
       (-> -Nat -Int B : (-FSb> -top (-filter -NonNegFixnum 1)))
       (-> -Int -Nat B : (-FSb> (-filter -PosFixnum 0) -top))
       (-> -NonPosInt -Int B : (-FSb> (-filter -NegFixnum 1) -top))
       (-> -Int -NegInt B : (-FSb> -top (-filter -NegFixnum 0)))
       (-> -Int -NonPosInt B : (-FSb> -top (-filter -NonPosFixnum 0)))
       (-> -Int -Int B : (-FSb> -top -top)))))
  (define fx<=-type
    (lambda ()
      (fx-from-cases
       (-> -Int -One B : (-FSb<= (-filter (Un -NonPosFixnum -One) 0) (-filter -PosFixnum 0)))
       (-> -One -Int B : (-FSb<= (-filter -PosFixnum 1) (-filter -NonPosFixnum 1)))
       (-> -Int -Zero B : (-FSb<= (-filter -NonPosFixnum 0) (-filter -PosFixnum 0)))
       (-> -Zero -Int B : (-FSb<= (-filter -NonNegFixnum 1) (-filter -NegFixnum 1)))

       (-> -PosByte -Byte B : (-FSb<= (-filter -PosByte 1) -top))
       (-> -Byte -Byte B : (-FSb<= -top (-filter -PosByte 0)))
       (-> -Pos -Byte B : (-FSb<= (-and (-filter -PosByte 0) (-filter -PosByte 1)) -top))
       (-> -Byte -Pos B : (-FSb<= -top (-and (-filter -PosByte 0) (-filter -PosByte 1))))
       (-> -Byte -Nat B : (-FSb<= -top (-and (-filter -PosByte 0) (-filter -Byte 1))))
       (-> -PosIndex -Index B : (-FSb<= (-filter -PosIndex 1) -top))
       (-> -Index -Index B : (-FSb<= -top (-filter -PosIndex 0)))
       (-> -Pos -Index B : (-FSb<= (-and (-filter -PosIndex 0) (-filter -PosIndex 1)) -top))
       (-> -Index -Pos B : (-FSb<= -top (-and (-filter -PosIndex 0) (-filter -PosIndex 1))))
       (-> -Nat -Byte B : (-FSb<= (-filter -Byte 0) -top))
       (-> -Nat -Index B : (-FSb<= (-filter -Index 0) -top))
       (-> -Index -Nat B : (-FSb<= -top (-and (-filter -PosIndex 0) (-filter -Index 1))))
       ;; general integer cases
       (-> -PosInt -Int B : (-FSb<= (-filter -PosFixnum 1) -top))
       (-> -Int -Nat B : (-FSb<= -top (-filter -PosFixnum 0)))
       (-> -Nat -Int B : (-FSb<= (-filter -NonNegFixnum 1) -top))
       (-> -Int -NegInt B : (-FSb<= (-filter -NegFixnum 0) -top))
       (-> -Int -NonPosInt B : (-FSb<= (-filter -NonPosFixnum 0) -top))
       (-> -NonPosInt -Int B : (-FSb<= -top (-filter -NegFixnum 1)))
       (-> -Int -Int B : (-FSb<= -top -top)))))
  (define fx>=-type
    (lambda ()
      (fx-from-cases
       (-> -One -Int B : (-FSb>= (-filter (Un -One -NonPosInt) 1) (-filter -PosFixnum 1)))
       (-> -Int -One B : (-FSb>= (-filter -PosFixnum 0) (-filter -NonPosFixnum 0)))
       (-> -Zero -Int B : (-FSb>= (-filter -NonPosFixnum 1) (-filter -PosFixnum 1)))
       (-> -Int -Zero B : (-FSb>= (-filter -NonNegFixnum 0) (-filter -NegFixnum 0)))

       (-> -Byte -PosByte B : (-FSb>= (-filter -PosByte 0) -top))
       (-> -Byte -Byte B : (-FSb>= -top (-filter -PosByte 1)))
       (-> -Byte -Pos B : (-FSb>= (-and (-filter -PosByte 1) (-filter -PosByte 0)) -top))
       (-> -Pos -Byte B : (-FSb>= -top (-and (-filter -PosByte 1) (-filter -PosByte 0))))
       (-> -Byte -Nat B : (-FSb>= (-filter -Byte 1) -top))
       (-> -Zero -Index B : (-FSb>= (-filter -Zero 1) (-filter -PosIndex 1)))
       (-> -Index -PosIndex B : (-FSb>= (-filter -PosIndex 0) -top))
       (-> -Index -Index B : (-FSb>= -top (-filter -PosIndex 1)))
       (-> -Index -Pos B : (-FSb>= (-and (-filter -PosIndex 0) (-filter -PosIndex 1)) -top))
       (-> -Pos -Index B : (-FSb>= -top (-and (-filter -PosIndex 0) (-filter -PosIndex 1))))
       (-> -Index -Nat B : (-FSb>= (-filter -Index 1) -top))
       (-> -Nat -Byte B : (-FSb>= -top (-and (-filter -Byte 0) (-filter -PosByte 1))))
       (-> -Nat -Index B : (-FSb>= -top (-and (-filter -Index 0) (-filter -PosIndex 1))))
       ;; general integer cases
       (-> -Int -PosInt B : (-FSb>= (-filter -PosFixnum 0) -top))
       (-> -Nat -Int B : (-FSb>= -top (-filter -PosFixnum 1)))
       (-> -Int -Nat B : (-FSb>= (-filter -NonNegFixnum 0) -top))
       (-> -NegInt -Int B : (-FSb>= (-filter -NegFixnum 1) -top))
       (-> -NonPosInt -Int B : (-FSb>= (-filter -NonPosFixnum 1) -top))
       (-> -Int -NonPosInt B : (-FSb>= -top (-filter -NegFixnum 0)))
       (-> -Int -Int B : (-FSb>= -top -top)))))
  (define fxmin-type
    (lambda ()
      (fx-from-cases
  ...
  (define (ret-nat-lt-arg t arg-num)
    (-refine x t
             (-SLI (-leq (-lexp 0)
                         (-lexp (list 1 (-id-path x))))
                   (-lt (-lexp (list 1 (-id-path x)))
  ...
  (define (bdiff int-type) (-unsafe-refine int-type
                                    (-eqSLI
                                     ;; return value
                                     (-lexp (list 1 (-lvl-arg-obj 0 0)))
                                     ;; difference of arguments 0 and 1
                                     (-lexp (list 1 (-lvl-arg-obj 1 0))
                                            (list -1 (-lvl-arg-obj 1 1))))))
  ...
  ;; special cases have been added to separate out the integer to integer
  ;; comparisons to add the linear inequalities they imply to their propositions
  (define (<-int-type-pattern)
    (list (-> -Int -Zero B : (-FSb< (-filter -NegInt 0) (-filter -Nat 0)))
          (-> -Zero -Int B : (-FSb< (-filter -PosInt 1) (-filter -NonPosInt 1)))
          (-> -Int -PosInt B : (-FSb< -top (-filter -PosInt 0)))
          (-> -Int -PosReal B : (-FS -top (-filter -PosInt 0)))
          (-> -Int -Nat B : (-FSb< -top (-filter -Nat 0)))
          (-> -Int -NonNegReal B : (-FS -top (-filter -Nat 0)))
          (-> -Nat -Int B : (-FSb< (-filter -PosInt 1) -top))
          (-> -NonNegReal -Int B : (-FS (-filter -PosInt 1) -top))
          (-> -Int -NonPosInt B : (-FSb< (-filter -NegInt 0) -top))
          (-> -Int -NonPosReal B : (-FS (-filter -NegInt 0) -top))
          (-> -NegInt -Int B : (-FSb< -top (-filter -NegInt 1)))
          (-> -NegReal -Int B : (-FS -top (-filter -NegInt 1)))
          (-> -NonPosInt -Int B : (-FSb< -top (-filter -NonPosInt 1)))
          (-> -NonPosReal -Int B : (-FS -top (-filter -NonPosInt 1)))))
  ...
  (define (>-int-type-pattern)
    (list (-> -Int -Zero B : (-FSb> (-filter -PosInt 0) (-filter -NonPosInt 0)))
          (-> -Zero -Int B : (-FSb> (-filter -NegInt 1) (-filter -Nat 1)))
          (-> -Int -Nat B : (-FSb> (-filter -PosInt 0) -top))
          (-> -Int -NonNegReal B : (-FS (-filter -PosInt 0) -top))
          (-> -PosInt -Int B : (-FSb> -top (-filter -PosInt 1)))
          (-> -PosReal -Int B : (-FS -top (-filter -PosInt 1)))
          (-> -Nat -Int B : (-FSb> -top (-filter -Nat 1)))
          (-> -NonNegReal -Int B : (-FS -top (-filter -Nat 1)))
          (-> -NonPosInt -Int B : (-FSb> (-filter -NegInt 1) -top))
          (-> -NonPosReal -Int B : (-FS (-filter -NegInt 1) -top))
          (-> -Int -NegInt B : (-FSb> -top (-filter -NegInt 0)))
          (-> -Int -NegReal B : (-FS -top (-filter -NegInt 0)))
          (-> -Int -NonPosInt B : (-FSb> -top (-filter -NonPosInt 0)))
          (-> -Int -NonPosReal B : (-FS -top (-filter -NonPosInt 0)))))
  (define (>-type-pattern base pos non-neg neg non-pos [zero -RealZero])
  ...
  (define (<=-int-type-pattern)
    (list (-> -Int -Zero B : (-FSb<= (-filter -NonPosInt 0) (-filter -PosInt 0)))
          (-> -Zero -Int B : (-FSb<= (-filter -Nat 1) (-filter -NegInt 1)))
          (-> -Int -Nat B : (-FSb<= -top (-filter -PosInt 0)))
          (-> -Int -NonNegReal B : (-FS -top (-filter -PosInt 0)))
          (-> -PosInt -Int B : (-FSb<= (-filter -PosInt 1) -top))
          (-> -PosReal -Int B : (-FS (-filter -PosInt 1) -top))
          (-> -Nat -Int B : (-FSb<= (-filter -Nat 1) -top))
          (-> -NonNegReal -Int B : (-FS (-filter -Nat 1) -top))
          (-> -NonPosInt -Int B : (-FSb<= -top (-filter -NegInt 1)))
          (-> -NonPosReal -Int B : (-FS -top (-filter -NegInt 1)))
          (-> -Int -NegInt B : (-FSb<= (-filter -NegInt 0) -top))
          (-> -Int -NegReal B : (-FS (-filter -NegInt 0) -top))
          (-> -Int -NonPosInt B : (-FSb<= (-filter -NonPosInt 0) -top))
          (-> -Int -NonPosReal B : (-FS (-filter -NonPosInt 0) -top))))
  (define (<=-type-pattern base pos non-neg neg non-pos [zero -RealZero])
  ...
  (define (>=-int-type-pattern)                    
    (list (-> -Int -Zero B : (-FSb>= (-filter -Nat 0) (-filter -NegInt 0)))
          (-> -Zero -Int B : (-FSb>= (-filter -NonPosInt 1) (-filter -PosInt 1)))
          (-> -Int -PosInt B : (-FSb>= (-filter -PosInt 0) -top))
          (-> -Int -PosReal B : (-FS (-filter -PosInt 0) -top))
          (-> -Int -Nat B : (-FSb>= (-filter -Nat 0) -top))
          (-> -Int -NonNegReal B : (-FS (-filter -Nat 0) -top))
          (-> -Nat -Int B : (-FSb>= -top (-filter -PosInt 1)))
          (-> -NonNegReal -Int B : (-FS -top (-filter -PosInt 1)))
          (-> -Int -NonPosInt B : (-FSb>= -top (-filter -NegInt 0)))
          (-> -Int -NonPosReal B : (-FS -top (-filter -NegInt 0)))
          (-> -NegInt -Int B : (-FSb>= (-filter -NegInt 1) -top))
          (-> -NegReal -Int B : (-FS (-filter -NegInt 1) -top))
          (-> -NonPosInt -Int B : (-FSb>= (-filter -NonPosInt 1) -top))
          (-> -NonPosReal -Int B : (-FS (-filter -NonPosInt 1) -top))))
  (define (>=-type-pattern base pos non-neg neg non-pos [zero -RealZero])
...
[=
 (from-cases
  ...
  (-> -Int -Int -Int B : (-FSt= -top -top))
  (->* (list N N) N B))]

[<  (from-cases
     (-> -Int -One B : (-FSb< (-filter -NonPosInt 0) (-filter -PosInt 0)))
     (-> -Int -Zero B : (-FSb< (-filter -NegInt 0) (-filter -Nat 0))) ; new amk
     (-> -Real -Zero B : (-FS (-filter -NegReal 0) (-filter -NonNegReal 0)))
     (-> -Zero -Int B : (-FSb< (-filter -PosInt 1) (-filter -NonPosInt 1))) ; new amk
     ...
     (-> -Byte -PosByte B : (-FSb< -top (-filter -PosByte 0)))
     (-> -Byte -Byte B : (-FSb< (-filter -PosByte 1) -top))
     (-> -PosInt -Byte B : (-FSb< (-and (-filter -PosByte 0) (-filter -PosByte 1)) -top))
     (-> -PosReal -Byte B : (-FS (-filter -PosByte 1) -top)) ; -PosReal is ok here, no filter for #f
     (-> -Byte -PosInt B : (-FSb< -top (-and (-filter -PosByte 0) (-filter -PosByte 1))))
     ...
     (-> -Index -PosIndex B : (-FSb< -top (-filter -PosIndex 0)))
     (-> -Index -Index B : (-FSb< (-filter -PosIndex 1) -top))
     (-> -PosInt -Index B : (-FSb< (-and (-filter -PosIndex 0) (-filter -PosIndex 1)) -top))
     (-> -PosReal -Index B : (-FS (-filter -PosIndex 1) -top))
     (-> -Index -PosInt B : (-FSb< -top (-and (-filter -PosIndex 0) (-filter -PosIndex 1))))
     (-> -Index -PosRat B : (-FS -top (-filter -PosIndex 0))) ; can't be -PosReal, which includes NaN
     (-> -Nat -Index B : (-FSb< (-and (-filter -Index 0) (-filter -PosIndex 1)) -top))
     (-> -NonNegReal -Index B : (-FS (-filter -PosIndex 1) -top))
     (-> -Index -Nat B : (-FSb< -top (-filter -Index 1)))
     (-> -Fixnum -PosInt B : (-FSb< -top (-and (-filter -PosFixnum 0) (-filter -PosFixnum 1))))
     (-> -Fixnum -PosRat B : (-FS -top (-filter -PosFixnum 0)))
     (-> -Fixnum -Nat B : (-FSb< -top (-and (-filter -NonNegFixnum 0) (-filter -NonNegFixnum 1))))
     (-> -Fixnum -NonNegRat B : (-FS -top (-filter -NonNegFixnum 0)))
     (-> -Nat -Fixnum B : (-FSb< (-and (-filter -PosFixnum 1) (-filter -NonNegFixnum 0)) -top))
     (-> -NonNegReal -Fixnum B : (-FS (-filter -PosFixnum 1) -top))
     (-> -Fixnum -NonPosInt B : (-FSb< (-and (-filter -NegFixnum 0) (-filter -NonPosFixnum 1)) -top))
     (-> -Fixnum -NonPosReal B : (-FS (-filter -NegFixnum 0) -top))
     (-> -NegInt -Fixnum B : (-FSb< -top (-and (-filter -NegFixnum 0) (-filter -NegFixnum 1))))
     (-> -NegRat -Fixnum B : (-FS -top (-filter -NegFixnum 1)))
     (-> -NonPosInt -Fixnum B : (-FSb< -top (-and (-filter -NonPosFixnum 0) (-filter -NonPosFixnum 1))))
     ...
     (<-int-type-pattern);(<-type-pattern -Int -PosInt -Nat -NegInt -NonPosInt -Zero)
     (-> -Int -Int B : (-FSb< -top -top))
     ...
     (->* (list R R) R B))]
[>  (from-cases
     (-> -One -Int B : (-FSb> (-filter -NonPosInt 1) (-filter -PosInt 1)))
     (-> -Int -Zero B : (-FSb> (-filter -PosInt 0) (-filter -NonPosInt 0)))
     (-> -Real -Zero B : (-FS (-filter -PosReal 0) (-filter -NonPosReal 0)))
     (-> -Zero -Int B : (-FSb> (-filter -NegInt 1) (-filter -NonNegReal 1)))
     ...
     (-> -PosByte -Byte B : (-FSb> -top (-filter -PosByte 1)))
     (-> -Byte -Byte B : (-FSb> (-filter -PosByte 0) -top))
     (-> -Byte -PosInt B : (-FSb> (-and (-filter -PosByte 0) (-filter -PosByte 1)) -top))
     (-> -Byte -PosReal B : (-FS (-filter -PosByte 0) -top))
     (-> -PosInt -Byte B : (-FSb> -top (-and (-filter -PosByte 0) (-filter -PosByte 1))))
     ...
     (-> -PosIndex -Index B : (-FSb> -top (-filter -PosIndex 1)))
     (-> -Index -Index B : (-FSb> (-filter -PosIndex 0) -top))
     (-> -Index -PosInt B : (-FSb> (-and (-filter -PosIndex 0) (-filter -PosIndex 1)) -top))
     (-> -Index -PosReal B : (-FS (-filter -PosIndex 0) -top))
     (-> -PosInt -Index B : (-FSb> -top (-and (-filter -PosIndex 0) (-filter -PosIndex 1))))
     (-> -PosRat -Index B : (-FS -top (-filter -PosIndex 1)))
     (-> -Index -Nat B : (-FSb> (-and (-filter -PosIndex 0) (-filter -Index 1)) -top))
     (-> -Index -NonNegReal B : (-FS (-filter -PosIndex 0) -top))
     (-> -Nat -Index B : (-FSb> -top (-filter -Index 0)))
     (-> -PosInt -Fixnum B : (-FSb> -top (-and (-filter -PosFixnum 0) (-filter -PosFixnum 1))))
     (-> -PosRat -Fixnum B : (-FS -top (-filter -PosFixnum 1)))
     (-> -Nat -Fixnum B : (-FSb> -top (-and (-filter -NonNegFixnum 0) (-filter -NonNegFixnum 1))))
     (-> -NonNegRat -Fixnum B : (-FS -top (-filter -NonNegFixnum 1)))
     (-> -Fixnum -Nat B : (-FSb> (-and (-filter -PosFixnum 0) (-filter -NonNegFixnum 1)) -top))
     (-> -Fixnum -NonNegReal B : (-FS (-filter -PosFixnum 0) -top))
     (-> -NonPosInt -Fixnum B : (-FSb> (-and (-filter -NonPosFixnum 0) (-filter -NegFixnum 1)) -top))
     (-> -NonPosReal -Fixnum B : (-FS (-filter -NegFixnum 1) -top))
     (-> -Fixnum -NegInt B : (-FSb> -top (-and (-filter -NegFixnum 0) (-filter -NegFixnum 1))))
     (-> -Fixnum -NegRat B : (-FS -top (-filter -NegFixnum 0)))
     (-> -Fixnum -NonPosInt B : (-FSb> -top (-and (-filter -NonPosFixnum 0) (-filter -NonPosFixnum 1))))
...
[<= (from-cases
     (-> -Int -One B : (-FSb<= (-filter (Un -NonPosInt -One) 0) (-filter -PosInt 0)))
     (-> -One -Int B : (-FSb<= (-filter -PosInt 1) (-filter -NonPosInt 1)))
     (-> -Int -Zero B : (-FSb<= (-filter -NonPosInt 0) (-filter -PosInt 0))) ; new amk
     (-> -Real -Zero B : (-FS (-filter -NonPosReal 0) (-filter -PosReal 0)))
     (-> -Zero -Int B : (-FSb<= (-filter -NonNegReal 1) (-filter -NegReal 1))) ; new amk
     ...
     (-> -PosInt -Fixnum B : (-FSb<= (-and (-filter -PosFixnum 0) (-filter -PosFixnum 1)) -top))
     (-> -PosReal -Fixnum B : (-FS (-filter -PosFixnum 1) -top))
     (-> -Nat -Fixnum B : (-FSb<= (-and (-filter -NonNegFixnum 0) (-filter -NonNegFixnum 1)) -top))
     (-> -NonNegReal -Fixnum B : (-FS (-filter -NonNegFixnum 1) -top))
     (-> -Fixnum -Nat B : (-FSb<= -top (-and (-filter -PosFixnum 0) (-filter -NonNegFixnum 1))))
     (-> -Fixnum -NonNegRat B : (-FS -top (-filter -PosFixnum 0)))
     (-> -NonPosInt -Fixnum B : (-FSb<= -top (-and (-filter -NonPosFixnum 0) (-filter -NegFixnum 1))))
     (-> -NonPosRat -Fixnum B : (-FS -top (-filter -NegFixnum 1)))
     (-> -Fixnum -NegInt B : (-FSb<= (-and (-filter -NegFixnum 0) (-filter -NegFixnum 1)) -top))
     (-> -Fixnum -NegReal B : (-FS (-filter -NegFixnum 0) -top))
     (-> -Fixnum -NonPosInt B : (-FSb<= (-and (-filter -NonPosFixnum 0) (-filter -NonPosFixnum 1)) -top))
...
[>= (from-cases
     ...
     (-> -Fixnum -PosInt B : (-FSb>= (-and (-filter -PosFixnum 0) (-filter -PosFixnum 1)) -top))
     (-> -Fixnum -PosReal B : (-FS (-filter -PosFixnum 0) -top))
     (-> -Fixnum -Nat B : (-FSb>= (-and (-filter -NonNegFixnum 0) (-filter -NonNegFixnum 1)) -top))
     (-> -Fixnum -NonNegReal B : (-FS (-filter -NonNegFixnum 0) -top))
     (-> -Nat -Fixnum B : (-FSb>= -top (-and (-filter -NonNegFixnum 0) (-filter -PosFixnum 1))))
     (-> -NonNegRat -Fixnum B : (-FS -top (-filter -PosFixnum 1)))
     (-> -Fixnum -NonPosInt B : (-FSb>= -top (-and (-filter -NegFixnum 0) (-filter -NonPosFixnum 1))))
     (-> -Fixnum -NonPosRat B : (-FS -top (-filter -NegFixnum 0)))
     (-> -NegInt -Fixnum B : (-FSb>= (-and (-filter -NegFixnum 0) (-filter -NegFixnum 1)) -top))
     (-> -NegReal -Fixnum B : (-FS (-filter -NegFixnum 1) -top))
     (-> -NonPosInt -Fixnum B : (-FSb>= (-and (-filter -NonPosFixnum 0) (-filter -NonPosFixnum 1)) -top))
...
[quotient/remainder ;; TODO(amk) can we bound quotient usefully as well?
 (from-cases
...
[abs (from-cases abs-cases)] ;; TODO(amk)?
